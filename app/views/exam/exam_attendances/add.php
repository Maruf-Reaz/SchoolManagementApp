...
<script>
    ...
    $(document).ready(function () {
        $("#mark_distribution").change(function () {
            ...
            // $("#data_table_body").empty();
            var dataString = 'class_id=' + class_id + '&exam_id=' + exam_id + '&subject_id=' + subject_id + '&section_id='
                + section_id + '&mark_distribution=' + mark_distribution;
            $.ajax
            ({
                type: "GET",
                dataType: 'json',
                    ...
                    if (objects.length === 0) {
                        ...
                        $.each(objects, function (key, value) {
                            $("#data_table_body").append('<tr>' +
                                ...
                                "<td>" +
                                "<button onclick='changeStatus(this.value," + value.student_id + "," + value.attendance_value + ")'value='" + value.id + "' class='btn btn-primary'> " + value.attendance_value + "</button>" +
                                "</td>" +
                                '</tr>'
                            )
                        })
    ...
    function changeStatus(attendance_id, student_id, attendance_value) {
        var exam_id = $("#exams").val();
        var class_id = $("#class").val();
        var section_id = $("#section").val();
        var subject_id = $("#subject").val();
        ...
        if (attendance_value === 1) {
            $.alert({
                title: 'Notice!',
                content: 'If the student is marked already Status will not be changed',
                typeAnimated: true,
                theme: 'supervan',
            });
        }
        ...
        var dataString = 'class_id=' + class_id + '&exam_id=' + exam_id + '&subject_id=' + subject_id + '&section_id='
            + section_id + '&mark_distribution=' + mark_distribution + '&attendance_id=' + attendance_id + '&student_id=' + student_id;
        $.ajax
        ({
            type: "GET",
            dataType: 'json',
            url: "/exam/examattendances/changeStatus",
            data: dataString,
            cache: false,
            success: function (objects) {
                console.log(objects)
